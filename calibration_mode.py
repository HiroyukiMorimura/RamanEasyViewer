...
@author: hiroy
Enhanced Integrated Raman Spectrum Analysis Tool with Peak Analysis, Q&A Feature, and Calibration
"""
...
def main():
    ...
    if analysis_mode == "スペクトル解析":
        ...
        **検量線作成モード:**
        ...
        3. **濃度データ確定**: 濃度データ確定ボタンを押して入力を確定
        4. **検量線タイプ選択**: ピーク面積または多変量解析を選択
        5. **波数範囲設定**: 解析に使用する波数範囲を指定
        6. **検量線作成実行**: 統計解析により検量線を作成
        7. **結果確認**: R²、RMSE等の統計指標を確認
        8. **結果エクスポート**: 検量線データをCSVでダウンロード
